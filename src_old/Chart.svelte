...
<script lang="ts">
    ...
    const calculateDeltaDewTemp = (
        ...
        const tsValues: TSValue[] = ts.map((ts, i) => {
                return { ts, diff: (real_temp[i] - dew_temp[i]), hum: (humidity[i]/(-10) + 10)};
        });
        return tsValues;
    };

    const drawTheGraph = (lineData: TSValue[], midnights: Timestamp[]) => {
        const importantValuesTemp = [-4, -2, 0, 2, 4, 6, 8, 10];
        const importantValuesHum = [0, 2, 4, 6, 8, 10];
        ...
        const yAxis = d3
            .axisLeft(yScale).tickValues(importantValuesTemp).tickFormat(d => `${d} t°`);



        // Draw horizontal lines for each important value
        importantValuesTemp.forEach(value => {
            innerSvg
                .append('line')
                .attr('x1', 0)
